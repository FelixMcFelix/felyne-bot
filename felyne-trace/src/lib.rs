...
pub use self::{event::*, extension::*, label::*, traces::FelyneTrace};
...
pub async fn read_async<R: AsyncBufRead + Unpin>(reader: R) -> IoResult<FelyneTrace> {
	...
	futures::StreamExt::next(&mut bin_shell).await
		.transpose()
		.map_err(|e| IoError::new(ErrorKind::InvalidData, e))
		.and_then(|opt| opt.ok_or_else(|| IoError::new(ErrorKind::InvalidData, "No decodable trace found!")))
}
...
pub async fn write_async<W: AsyncWrite + Unpin>(mut writer: W, trace: &FelyneTrace) -> IoResult<W> {
	...
	futures::SinkExt::send(&mut bin_shell, trace).await
		.map_err(|e| IoError::new(ErrorKind::InvalidData, e))?;

	write_shell.shutdown().await
		.map(|_| writer)
}
